...
const binaryName =
  process.platform === "win32" ? "filter_cancelled.exe" : "filter_cancelled";

function binaryPath(): string {
  return join(process.cwd(), "native", "filters", binaryName);
}

export function filterCancelledIndicesTypeScript(statuses: RequestStatus[]): number[] {
  const out: number[] = [];
...
export async function filterCancelledIndicesCpp(
  statuses: RequestStatus[]
): Promise<number[]> {
  const path = binaryPath();
  if (!existsSync(path)) {
    return filterCancelledIndicesTypeScript(statuses);
  }
